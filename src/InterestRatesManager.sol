...
/// @dev This contract inherits from EvoqStorage so that Evoq can delegate calls to this contract.
contract InterestRatesManager is IInterestRatesManager, EvoqStorage {
    ...
    function updateP2PIndexes(address _poolToken) external {
        ...
        uint256 poolSupplyIndex = IVToken(_poolToken).exchangeRateCurrent();
        uint256 poolBorrowIndex = IVToken(_poolToken).borrowIndex();
        ...
        emit P2PIndexesUpdated(_poolToken, newP2PSupplyIndex, newP2PBorrowIndex, poolSupplyIndex, poolBorrowIndex);
    }
