...
abstract contract MatchingEngine is EvoqUtils {
    ...
    /// @notice Matches suppliers' liquidity waiting on Venus up to the given `_amount` and moves it to peer-to-peer.
    /// @dev Note: This function expects Venus's exchange rate and peer-to-peer indexes to have been updated.
        ...
        returns (uint256 matched, uint256 gasConsumedInMatching)
    {
        ...
        vars.poolIndex = IVToken(_poolToken).exchangeRateStored(); // Exchange rate has already been updated.
        vars.p2pIndex = p2pSupplyIndex[_poolToken];
        address firstPoolSupplier;
        vars.gasLeftAtTheBeginning = gasleft();

        while (
            matched < _amount && (firstPoolSupplier = suppliersOnPool[_poolToken].getHead()) != address(0)
                && vars.gasLeftAtTheBeginning - gasleft() < _maxGasForMatching
        ) {
            Types.SupplyBalance storage firstPoolSupplierBalance = supplyBalanceInOf[_poolToken][firstPoolSupplier];
            ...
            if (vars.inUnderlying <= maxToMatch) {
                ...
                p2pSupplyBalance = firstPoolSupplierBalance.inP2P + vars.inUnderlying.div(vars.p2pIndex);
                matched += vars.inUnderlying;
            } else {
                poolSupplyBalance = firstPoolSupplierBalance.onPool - maxToMatch.div(vars.poolIndex);
            ...
            emit SupplierPositionUpdated(firstPoolSupplier, _poolToken, poolSupplyBalance, p2pSupplyBalance);
        }

        gasConsumedInMatching = vars.gasLeftAtTheBeginning - gasleft();
    }

    /// @notice Unmatches suppliers' liquidity in peer-to-peer up to the given `_amount` and moves it to Venus.
    /// @dev Note: This function expects Venus's exchange rate and peer-to-peer indexes to have been updated.
        ...
        returns (uint256)
    {
        ...
        while (
            remainingToUnmatch > 0 && (firstP2PSupplier = suppliersInP2P[_poolToken].getHead()) != address(0)
                && vars.gasLeftAtTheBeginning - gasleft() < _maxGasForMatching
        ) {
            Types.SupplyBalance storage firstP2PSupplierBalance = supplyBalanceInOf[_poolToken][firstP2PSupplier];
            ...
            if (vars.inUnderlying <= remainingToUnmatch) {
                ...
                poolSupplyBalance = firstP2PSupplierBalance.onPool + vars.inUnderlying.div(vars.poolIndex);
                remainingToUnmatch -= vars.inUnderlying;
            } else {
                poolSupplyBalance = firstP2PSupplierBalance.onPool + remainingToUnmatch.div(vars.poolIndex);
                p2pSupplyBalance = firstP2PSupplierBalance.inP2P - remainingToUnmatch.div(vars.p2pIndex);
            ...
            emit SupplierPositionUpdated(firstP2PSupplier, _poolToken, poolSupplyBalance, p2pSupplyBalance);
        }

        return _amount - remainingToUnmatch;
    }

    /// @notice Matches borrowers' liquidity waiting on Venus up to the given `_amount` and moves it to peer-to-peer.
        ...
        returns (uint256 matched, uint256 gasConsumedInMatching)
    {
        ...
        while (
            matched < _amount && (firstPoolBorrower = borrowersOnPool[_poolToken].getHead()) != address(0)
                && vars.gasLeftAtTheBeginning - gasleft() < _maxGasForMatching
        ) {
            ...
            if (vars.inUnderlying <= maxToMatch) {
                ...
                p2pBorrowBalance = firstPoolBorrowerBalance.inP2P + vars.inUnderlying.div(vars.p2pIndex);
                matched += vars.inUnderlying;
            } else {
                poolBorrowBalance = firstPoolBorrowerBalance.onPool - maxToMatch.div(vars.poolIndex);
            ...
            emit BorrowerPositionUpdated(firstPoolBorrower, _poolToken, poolBorrowBalance, p2pBorrowBalance);
        }

        gasConsumedInMatching = vars.gasLeftAtTheBeginning - gasleft();
    }

    /// @notice Unmatches borrowers' liquidity in peer-to-peer for the given `_amount` and moves it to Venus.
        ...
        returns (uint256)
    {
        ...
        while (
            remainingToUnmatch > 0 && (firstP2PBorrower = borrowersInP2P[_poolToken].getHead()) != address(0)
                && vars.gasLeftAtTheBeginning - gasleft() < _maxGasForMatching
        ) {
            Types.BorrowBalance storage firstP2PBorrowerBalance = borrowBalanceInOf[_poolToken][firstP2PBorrower];
            ...
            if (vars.inUnderlying <= remainingToUnmatch) {
                ...
                poolBorrowBalance = firstP2PBorrowerBalance.onPool + vars.inUnderlying.div(vars.poolIndex);
                remainingToUnmatch -= vars.inUnderlying;
            } else {
                poolBorrowBalance = firstP2PBorrowerBalance.onPool + remainingToUnmatch.div(vars.poolIndex);
                p2pBorrowBalance = firstP2PBorrowerBalance.inP2P - remainingToUnmatch.div(vars.p2pIndex);
                remainingToUnmatch = 0;
            }
            ...
            emit BorrowerPositionUpdated(firstP2PBorrower, _poolToken, poolBorrowBalance, p2pBorrowBalance);
        }
    ...
    function _updateSupplierInDS(address _poolToken, address _user) internal {
        ...
        if (formerValueOnPool != onPool) {
            ...
            if (onPool > 0) {
                marketSuppliersOnPool.insertSorted(_user, onPool, maxSortedUsers);
            }
        }
        ...
        if (formerValueInP2P != inP2P) {
            ...
            if (inP2P > 0) {
                marketSuppliersInP2P.insertSorted(_user, inP2P, maxSortedUsers);
            }
        }
    ...
    function _updateBorrowerInDS(address _poolToken, address _user) internal {
        ...
        if (formerValueOnPool != onPool) {
            ...
            if (onPool > 0) {
                marketBorrowersOnPool.insertSorted(_user, onPool, maxSortedUsers);
            }
        }
        ...
        if (formerValueInP2P != inP2P) {
            ...
            if (inP2P > 0) {
                marketBorrowersInP2P.insertSorted(_user, inP2P, maxSortedUsers);
            }
        }
