...
/// @notice Main Logic of Evoq Protocol, implementation of the 5 main functionalities: supply, borrow, withdraw, repay and liquidate.
contract PositionsManager is IPositionsManager, MatchingEngine {
    ...
    /// @notice Thrown when the borrow on Venus failed and throws back the Venus error code.
    error BorrowOnVenusFailed(uint256 errorCode);

    /// @notice Thrown when the redeem on Venus failed and throws back the Venus error code.
    error RedeemOnVenusFailed(uint256 errorCode);

    /// @notice Thrown when the repay on Venus failed and throws back the Venus error code.
    error RepayOnVenusFailed(uint256 errorCode);

    /// @notice Thrown when the mint on Venus failed and throws back the Venus error code.
    error MintOnVenusFailed(uint256 errorCode);
    ...
    function supplyLogic(
        ...
        if (marketPauseStatus[_poolToken].isSupplyPaused) {
        ...
        if (vars.remainingToSupply > 0 && !p2pDisabled && borrowersOnPool[_poolToken].getHead() != address(0)) {
            (uint256 matched,) = _matchBorrowers(_poolToken, vars.remainingToSupply, _maxGasForMatching); // In underlying.
        ...
        Types.SupplyBalance storage supplierSupplyBalance = supplyBalanceInOf[_poolToken][_onBehalf];
        ...
        if (vars.remainingToSupply > 0) {
            supplierSupplyBalance.onPool += vars.remainingToSupply.div(
                IVToken(_poolToken).exchangeRateStored() // Exchange rate has already been updated.
            ); // In scaled balance.
        ...
        emit Supplied(_from, _onBehalf, _poolToken, _amount, supplierSupplyBalance.onPool, supplierSupplyBalance.inP2P);
        return _amount;
    }
    ...
    function borrowLogic(
        ...
        Types.Delta storage delta = deltas[_poolToken];
        vars.poolSupplyIndex = IVToken(_poolToken).exchangeRateStored(); // Exchange rate has already been updated.
        bool p2pDisabled = p2pDisabled[_poolToken];
        ...
        if (delta.p2pSupplyDelta > 0 && !p2pDisabled) {
            ...
            if (deltaInUnderlying > vars.remainingToBorrow) {
                vars.toWithdraw += vars.remainingToBorrow;
                delta.p2pSupplyDelta -= vars.remainingToBorrow.div(vars.poolSupplyIndex);
                vars.remainingToBorrow = 0;
            } else {
                vars.toWithdraw += deltaInUnderlying;
                delta.p2pSupplyDelta = 0;
                vars.remainingToBorrow -= deltaInUnderlying;
            }
        ...
        if (vars.remainingToBorrow > 0 && !p2pDisabled && suppliersOnPool[_poolToken].getHead() != address(0)) {
            (uint256 matched,) = _matchSuppliers(_poolToken, vars.remainingToBorrow, _maxGasForMatching); // In underlying.
        ...
        Types.BorrowBalance storage borrowerBorrowBalance = borrowBalanceInOf[_poolToken][_borrower];

        if (vars.toWithdraw > 0) {
            uint256 toAddInP2P = vars.toWithdraw.div(p2pBorrowIndex[_poolToken]); // In peer-to-peer unit.

            deltas[_poolToken].p2pBorrowAmount += toAddInP2P;
            borrowerBorrowBalance.inP2P += toAddInP2P;
            emit P2PAmountsUpdated(_poolToken, delta.p2pSupplyAmount, delta.p2pBorrowAmount);

            // If this value is equal to 0 the withdraw will revert on Venus.
            if (vars.toWithdraw.div(vars.poolSupplyIndex) > 0) {
                _withdrawFromPool(_poolToken, vars.toWithdraw);
            } // Reverts on error.
        }
        ...
        // Borrow on pool.
        if (vars.remainingToBorrow > 0) {
            borrowerBorrowBalance.onPool += vars.remainingToBorrow.div(lastPoolIndexes[_poolToken].lastBorrowPoolIndex); // In pool borrow unit.
            _borrowFromPool(_poolToken, vars.remainingToBorrow);
        }

        _updateBorrowerInDS(_poolToken, _borrower);
        underlyingToken.safeTransfer(_receiver, _amount);

        emit Borrowed(msg.sender, _poolToken, _amount, borrowerBorrowBalance.onPool, borrowerBorrowBalance.inP2P);
        return _amount;
    }
    ...
    function withdrawLogic(
        ...
        if (_receiver == address(0)) revert AddressIsZero();
        if (!marketStatus[_poolToken].isCreated) revert MarketNotCreated();
        if (marketPauseStatus[_poolToken].isWithdrawPaused) {
            revert WithdrawIsPaused();
        }
        ...
        if (_isLiquidatable(_supplier, _poolToken, toWithdraw, 0)) {
            revert UnauthorisedWithdraw();
        }

        return _unsafeWithdrawLogic(_poolToken, toWithdraw, _supplier, _receiver, _maxGasForMatching);
    }
    ...
    function repayLogic(
        ...
        if (!userMembership[_poolToken][_onBehalf]) {
            revert UserNotMemberOfMarket();
        }
        ...
        return _unsafeRepayLogic(_poolToken, _repayer, _onBehalf, toRepay, _maxGasForMatching);
    }
    ...
    function liquidateLogic(
        ...
        if (_amount == 0) revert AmountIsZero();
        if (!marketStatus[_poolTokenCollateral].isCreated) {
        ...
        if (!marketStatus[_poolTokenBorrowed].isCreated) {
            revert MarketNotCreated();
        }
        Types.MarketPauseStatus memory borrowPause = marketPauseStatus[_poolTokenBorrowed];
        if (borrowPause.isLiquidateBorrowPaused) {
            revert LiquidateBorrowIsPaused();
        }
        if (!userMembership[_poolTokenBorrowed][_borrower] || !userMembership[_poolTokenCollateral][_borrower]) {
            revert UserNotMemberOfMarket();
        }
        ...
        (vars.liquidationAllowed, vars.closeFactor) = _liquidationAllowed(_borrower, borrowPause.isDeprecated);
        ...
        } // Same mechanism as Venus. Liquidator cannot repay more than part of the debt (cf close factor on Venus).

        _unsafeRepayLogic(_poolTokenBorrowed, msg.sender, _borrower, _amount, 0);

        IVenusOracle venusOracle = IVenusOracle(comptroller.oracle());
        vars.collateralPrice = venusOracle.getUnderlyingPrice(_poolTokenCollateral);
        vars.borrowedPrice = venusOracle.getUnderlyingPrice(_poolTokenBorrowed);
        if (vars.collateralPrice == 0 || vars.borrowedPrice == 0) {
            revert VenusOracleFailed();
        }
        ...
        vars.amountToSeize = Math.min(
            _amount.mul(liquidationIncentiveMantissa).mul(vars.borrowedPrice).div(vars.collateralPrice),
            _getUserSupplyBalanceInOf(_poolTokenCollateral, _borrower)
        );

        uint256 amountForTreasury;
        if (address(treasuryVault) != address(0) && treasuryPercentMantissa != 0) {
            amountForTreasury = vars.amountToSeize.mul(treasuryPercentMantissa).div(liquidationIncentiveMantissa);
            _unsafeWithdrawLogic(_poolTokenCollateral, amountForTreasury, _borrower, treasuryVault, 0);
        }

        uint256 amountForLiquidator = vars.amountToSeize - amountForTreasury;
        _unsafeWithdrawLogic(_poolTokenCollateral, amountForLiquidator, _borrower, msg.sender, 0);

        emit Liquidated(msg.sender, _borrower, _poolTokenBorrowed, _amount, _poolTokenCollateral, vars.amountToSeize);
        return (_amount, vars.amountToSeize);
    }

    /// @notice Implements increaseP2PDeltas logic.
    /// @dev The current Evoq supply on the pool might not be enough to borrow `_amount` before resupplying it.
    ...
    function increaseP2PDeltasLogic(address _poolToken, uint256 _amount) external isMarketCreated(_poolToken) {
        ...
        uint256 poolSupplyIndex = IVToken(_poolToken).exchangeRateStored();
        _amount = Math.min(
            _amount,
            Math.min(
    ...
    function _unsafeWithdrawLogic(
        ...
        Types.SupplyBalance storage supplierSupplyBalance = supplyBalanceInOf[_poolToken][_supplier];
        ...
        if (onPoolSupply > 0) {
            ...
            if (vars.remainingToWithdraw == 0) {
                ...
                emit Withdrawn(
                    _supplier, _receiver, _poolToken, _amount, supplierSupplyBalance.onPool, supplierSupplyBalance.inP2P
                );
        ...
        supplierSupplyBalance.inP2P -=
            Math.min(supplierSupplyBalance.inP2P, vars.remainingToWithdraw.div(vars.p2pSupplyIndex)); // In peer-to-peer supply unit.
        _updateSupplierInDS(_poolToken, _supplier);
        ...
        // Promote pool suppliers.
        if (
            vars.remainingToWithdraw > 0 && !p2pDisabled[_poolToken]
                && suppliersOnPool[_poolToken].getHead() != address(0)
        ) {
            (uint256 matched, uint256 gasConsumedInMatching) =
                _matchSuppliers(_poolToken, vars.remainingToWithdraw, vars.remainingGasForMatching);
            if (vars.remainingGasForMatching <= gasConsumedInMatching) {
                vars.remainingGasForMatching = 0;
            } else {
                vars.remainingGasForMatching -= gasConsumedInMatching;
            }
        ...
        if (vars.remainingToWithdraw > 0) {
            uint256 unmatched = _unmatchBorrowers(_poolToken, vars.remainingToWithdraw, vars.remainingGasForMatching);

            // Increase the peer-to-peer borrow delta.
            if (unmatched < vars.remainingToWithdraw) {
                delta.p2pBorrowDelta +=
                    (vars.remainingToWithdraw - unmatched).div(lastPoolIndexes[_poolToken].lastBorrowPoolIndex);
                emit P2PBorrowDeltaUpdated(_poolToken, delta.p2pBorrowDelta);
            }
    ...
    function _unsafeRepayLogic(
        ...
        if (lastBorrowBlock[_onBehalf] == block.number) {
            revert SameBlockBorrowRepay();
        }
        ...
        Types.BorrowBalance storage borrowerBorrowBalance = borrowBalanceInOf[_poolToken][_onBehalf];
        ...
        if (vars.borrowedOnPool > 0) {
            ...
            if (vars.maxToRepayOnPool > vars.remainingToRepay) {
                ...
                borrowerBorrowBalance.onPool -= Math.min(vars.borrowedOnPool, vars.toRepay.div(vars.poolBorrowIndex)); // In pool borrow unit.
                ...
                emit Repaid(
                    _repayer, _onBehalf, _poolToken, _amount, borrowerBorrowBalance.onPool, borrowerBorrowBalance.inP2P
                );
        ...
        borrowerBorrowBalance.inP2P -=
            Math.min(borrowerBorrowBalance.inP2P, vars.remainingToRepay.div(vars.p2pBorrowIndex)); // In peer-to-peer borrow unit.
        _updateBorrowerInDS(_poolToken, _onBehalf);
        ...
        if (vars.remainingToRepay > 0) {
            ...
            vars.feeToRepay = Math.zeroFloorSub(
                ...
                delta.p2pSupplyAmount.mul(vars.p2pSupplyIndex).zeroFloorSub(
                    delta.p2pSupplyDelta.mul(IVToken(_poolToken).exchangeRateStored())
                )
            );
        ...
        // Promote pool borrowers.
        if (
            vars.remainingToRepay > 0 && !p2pDisabled[_poolToken] && borrowersOnPool[_poolToken].getHead() != address(0)
        ) {
            (uint256 matched, uint256 gasConsumedInMatching) =
                _matchBorrowers(_poolToken, vars.remainingToRepay, vars.remainingGasForMatching);
            if (vars.remainingGasForMatching <= gasConsumedInMatching) {
                vars.remainingGasForMatching = 0;
            } else {
                vars.remainingGasForMatching -= gasConsumedInMatching;
        ...
        if (vars.remainingToRepay > 0) {
            uint256 unmatched = _unmatchSuppliers(_poolToken, vars.remainingToRepay, vars.remainingGasForMatching);
        ...
        emit Repaid(_repayer, _onBehalf, _poolToken, _amount, borrowerBorrowBalance.onPool, borrowerBorrowBalance.inP2P);
        return _amount;
    }

    /// @dev Supplies underlying tokens to Venus.
    ...
    function _supplyToPool(address _poolToken, ERC20 _underlyingToken, uint256 _amount) internal {
        if (_poolToken == vBnb) {
            IWBNB(wBnb).withdraw(_amount); // Turn wBNB into BNB.
            IVBnb(_poolToken).mint{value: _amount}();
        } else {
            _underlyingToken.safeApprove(_poolToken, _amount);
            uint256 errorCode = IVToken(_poolToken).mint(_amount);
            if (errorCode != 0) revert MintOnVenusFailed(errorCode);
        }
    }

    /// @dev Withdraws underlying tokens from Venus.
    /// @param _poolToken The address of the pool token.
    /// @param _amount The amount of token (in underlying).
    function _withdrawFromPool(address _poolToken, uint256 _amount) internal {
        ...
        _amount = Math.min(IVToken(_poolToken).balanceOfUnderlying(address(this)), _amount);

        uint256 errorCode = IVToken(_poolToken).redeemUnderlying(_amount);
        if (errorCode != 0) revert RedeemOnVenusFailed(errorCode);

        if (_poolToken == vBnb) IWBNB(address(wBnb)).deposit{value: _amount}(); // Turn the BNB received in wBNB.
    }

    /// @dev Borrows underlying tokens from Venus.
    /// @param _poolToken The address of the pool token.
    /// @param _amount The amount of token (in underlying).
    function _borrowFromPool(address _poolToken, uint256 _amount) internal {
        uint256 errorCode = IVToken(_poolToken).borrow(_amount);
        if (errorCode != 0) revert BorrowOnVenusFailed(errorCode);

        if (_poolToken == vBnb) IWBNB(address(wBnb)).deposit{value: _amount}(); // Turn the BNB received in wBNB.
    }

    /// @dev Repays underlying tokens to Venus.
    /// @param _poolToken The address of the pool token.
    ...
    function _repayToPool(address _poolToken, ERC20 _underlyingToken, uint256 _amount) internal {
        // Repay only what is necessary. The remaining tokens stays on the contracts and are claimable by the Treasury.
        _amount = Math.min(
            _amount,
            IVToken(_poolToken).borrowBalanceCurrent(address(this)) // The debt of the contract.
        );

        if (_amount > 0) {
            if (_poolToken == vBnb) {
                IWBNB(wBnb).withdraw(_amount); // Turn wBNB into BNB.
                IVBnb(_poolToken).repayBorrow{value: _amount}();
            } else {
                _underlyingToken.safeApprove(_poolToken, _amount);
                uint256 errorCode = IVToken(_poolToken).repayBorrow(_amount);
                if (errorCode != 0) revert RepayOnVenusFailed(errorCode);
            }
        }
    }
    ...
    function _leaveMarketIfNeeded(address _poolToken, address _user) internal {
        ...
        mapping(address => bool) storage userMembership = userMembership[_poolToken];
        if (
            userMembership[_user] && supplyBalance.inP2P == 0 && supplyBalance.onPool == 0 && borrowBalance.inP2P == 0
            ...
            if (index != length - 1) {
                enteredMarkets[index] = enteredMarkets[length - 1];
            }
            enteredMarkets.pop();
        }
    }
