...
/// @notice Intermediary layer exposing endpoints to query live data related to the Evoq Protocol market indexes & rates.
abstract contract IndexesLens is LensStorage {
    ...
    /// @dev If not virtually updated, the indexes returned are those used by Evoq for non-updated markets during the liquidity check.
    /// @param _poolToken The address of the market.
    ...
    {
        (poolSupplyIndex, poolBorrowIndex,) = _accruePoolInterests(IVToken(_poolToken));
    }
    ...
    {
        delta = evoq.deltas(_poolToken);
        Types.LastPoolIndexes memory lastPoolIndexes = evoq.lastPoolIndexes(_poolToken);

        if (!_updated) {
            indexes.poolSupplyIndex = IVToken(_poolToken).exchangeRateStored();
            indexes.poolBorrowIndex = IVToken(_poolToken).borrowIndex();
        } else {
            (indexes.poolSupplyIndex, indexes.poolBorrowIndex,) = _accruePoolInterests(IVToken(_poolToken));
        }

        (indexes.p2pSupplyIndex, indexes.p2pBorrowIndex,) = _computeP2PIndexes(
            _poolToken, _updated, indexes.poolSupplyIndex, indexes.poolBorrowIndex, delta, lastPoolIndexes
        );
    }
    ...
    /// @return poolSupplyIndex The supply index.
    /// @return poolBorrowIndex The borrow index.
    function _accruePoolInterests(IVToken _poolToken)
        internal
        view
        returns (uint256 poolSupplyIndex, uint256 poolBorrowIndex, PoolInterestsVars memory vars)
    {
        ...
        uint256 simpleInterestFactor = borrowRateMantissa * (block.number - accrualBlockNumberPrior);
        ...
        poolSupplyIndex = (vars.cash + vars.totalBorrows - vars.totalReserves).div(_poolToken.totalSupply());
    ...
    /// @param _lastPoolIndexes The last pool indexes stored on Evoq.
    ...
    {
        marketParameters = evoq.marketParameters(_poolToken);

        if (!_updated) {
            return (evoq.p2pSupplyIndex(_poolToken), evoq.p2pBorrowIndex(_poolToken), marketParameters);
        }

        InterestRatesModel.GrowthFactors memory growthFactors = InterestRatesModel.computeGrowthFactors(
            _poolSupplyIndex,
            _poolBorrowIndex,
            _lastPoolIndexes,
        ...
        _p2pSupplyIndex = InterestRatesModel.computeP2PIndex(
            InterestRatesModel.P2PIndexComputeParams({
                ...
                lastP2PIndex: evoq.p2pSupplyIndex(_poolToken),
        ...
        _p2pBorrowIndex = InterestRatesModel.computeP2PIndex(
            InterestRatesModel.P2PIndexComputeParams({
                ...
                lastP2PIndex: evoq.p2pBorrowIndex(_poolToken),
