...
/// @notice Intermediary layer exposing endpoints to query live data related to the Evoq Protocol markets.
abstract contract MarketsLens is RatesLens {
    ...
    function isMarketCreated(address _poolToken) external view returns (bool) {
        return evoq.marketStatus(_poolToken).isCreated;
    }
    ...
    function getAllMarkets() external view returns (address[] memory marketsCreated) {
        return evoq.getAllMarkets();
    }

    /// @notice For a given market, returns the average supply/borrow rates and amounts of underlying asset supplied and borrowed through Evoq, on the underlying pool and matched peer-to-peer.
    ...
    {
        (avgSupplyRatePerBlock, p2pSupplyAmount, poolSupplyAmount) = getAverageSupplyRatePerBlock(_poolToken);
        (avgBorrowRatePerBlock, p2pBorrowAmount, poolBorrowAmount) = getAverageBorrowRatePerBlock(_poolToken);
    }
        ...
        )
    {
        ...
        lastUpdateBlockNumber = evoq.lastPoolIndexes(_poolToken).lastUpdateBlockNumber;
    }
    ...
    /// @return collateralFactor The pool collateral factor also used by Evoq.
    ...
    {
        underlying = _poolToken == evoq.vBnb() ? evoq.wBnb() : IVToken(_poolToken).underlying();

        isCreated = evoq.marketStatus(_poolToken).isCreated;
        p2pDisabled = evoq.p2pDisabled(_poolToken);
    ...
    function getMarketPauseStatus(address _poolToken) external view returns (Types.MarketPauseStatus memory) {
        return evoq.marketPauseStatus(_poolToken);
    }
