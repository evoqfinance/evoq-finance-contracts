...
/// @title RatesLens.
/// @author Evoq Finance.
/// @custom:contact security@evoq.xyz
/// @notice Intermediary layer exposing endpoints to query live data related to the Evoq Protocol users and their positions.
abstract contract RatesLens is UsersLens {
    ...
    /// @dev Note: the returned supply rate is a low estimate: when supplying through Evoq-Venus,
    ...
    /// @return totalBalance The total balance supplied through Evoq (in underlying).
        ...
        returns (uint256 nextSupplyRatePerBlock, uint256 balanceOnPool, uint256 balanceInP2P, uint256 totalBalance)
    {
        ...
        Types.SupplyBalance memory supplyBalance = evoq.supplyBalanceInOf(_poolToken, _user);

        uint256 repaidToPool;
        if (!evoq.p2pDisabled(_poolToken)) {
            if (delta.p2pBorrowDelta > 0) {
                uint256 matchedDelta = Math.min(delta.p2pBorrowDelta.mul(indexes.poolBorrowIndex), _amount);
            ...
            if (_amount > 0) {
                address firstPoolBorrower = evoq.getHead(_poolToken, Types.PositionType.BORROWERS_ON_POOL);
                uint256 firstPoolBorrowerBalance = evoq.borrowBalanceInOf(_poolToken, firstPoolBorrower).onPool;

                uint256 matchedP2P = Math.min(firstPoolBorrowerBalance.mul(indexes.poolBorrowIndex), _amount);
        ...
        if (_amount > 0) {
            supplyBalance.onPool += _amount.div(indexes.poolSupplyIndex);
        }
        ...
        (nextSupplyRatePerBlock, totalBalance) =
            _getUserSupplyRatePerBlock(_poolToken, balanceInP2P, balanceOnPool, _amount, repaidToPool);
    }
    ...
    /// @dev Note: the returned borrow rate is a high estimate: when borrowing through Evoq-Venus,
        ...
        returns (uint256 nextBorrowRatePerBlock, uint256 balanceOnPool, uint256 balanceInP2P, uint256 totalBalance)
    {
        ...
        Types.BorrowBalance memory borrowBalance = evoq.borrowBalanceInOf(_poolToken, _user);

        uint256 withdrawnFromPool;
        if (!evoq.p2pDisabled(_poolToken)) {
            if (delta.p2pSupplyDelta > 0) {
                uint256 matchedDelta = Math.min(delta.p2pSupplyDelta.mul(indexes.poolSupplyIndex), _amount);
            ...
            if (_amount > 0) {
                address firstPoolSupplier = evoq.getHead(_poolToken, Types.PositionType.SUPPLIERS_ON_POOL);
                uint256 firstPoolSupplierBalance = evoq.supplyBalanceInOf(_poolToken, firstPoolSupplier).onPool;

                uint256 matchedP2P = Math.min(firstPoolSupplierBalance.mul(indexes.poolSupplyIndex), _amount);
        ...
        if (_amount > 0) {
            borrowBalance.onPool += _amount.div(indexes.poolBorrowIndex);
        }
        ...
        (nextBorrowRatePerBlock, totalBalance) =
            _getUserBorrowRatePerBlock(_poolToken, balanceInP2P, balanceOnPool, _amount, withdrawnFromPool);
    }
    ...
    {
        (uint256 balanceOnPool, uint256 balanceInP2P,) = getCurrentSupplyBalanceInOf(_poolToken, _user);

        (supplyRatePerBlock,) = _getUserSupplyRatePerBlock(_poolToken, balanceInP2P, balanceOnPool, 0, 0);
    }
    ...
    {
        (uint256 balanceOnPool, uint256 balanceInP2P,) = getCurrentBorrowBalanceInOf(_poolToken, _user);

        (borrowRatePerBlock,) = _getUserBorrowRatePerBlock(_poolToken, balanceInP2P, balanceOnPool, 0, 0);
    }
        ...
        returns (uint256 avgSupplyRatePerBlock, uint256 p2pSupplyAmount, uint256 poolSupplyAmount)
    {
        ...
        uint256 poolSupplyRate = vToken.supplyRatePerBlock();
        uint256 poolBorrowRate = vToken.borrowRatePerBlock();

        (Types.Delta memory delta, Types.Indexes memory indexes) = _getIndexes(_poolToken, true);

        Types.MarketParameters memory marketParams = evoq.marketParameters(_poolToken);
        ...
        uint256 p2pSupplyRate = InterestRatesModel.computeP2PSupplyRatePerBlock(
            InterestRatesModel.P2PRateComputeParams({
                p2pRate: PercentageMath.weightedAvg(poolSupplyRate, poolBorrowRate, marketParams.p2pIndexCursor),
        ...
        (p2pSupplyAmount, poolSupplyAmount) =
            _getMarketSupply(_poolToken, indexes.p2pSupplyIndex, indexes.poolSupplyIndex, delta);

        (avgSupplyRatePerBlock,) = _getWeightedRate(p2pSupplyRate, poolSupplyRate, p2pSupplyAmount, poolSupplyAmount);
    }
        ...
        returns (uint256 avgBorrowRatePerBlock, uint256 p2pBorrowAmount, uint256 poolBorrowAmount)
    {
        ...
        (avgBorrowRatePerBlock,) = _getWeightedRate(p2pBorrowRate, poolBorrowRate, p2pBorrowAmount, poolBorrowAmount);
    }
        ...
        returns (uint256 p2pSupplyRate, uint256 p2pBorrowRate, uint256 poolSupplyRate, uint256 poolBorrowRate)
    {
        ...
        ratesVars.delta = evoq.deltas(_poolToken);
        ratesVars.lastPoolIndexes = evoq.lastPoolIndexes(_poolToken);

        bool updated = _suppliedOnPool > 0 || _borrowedFromPool > 0 || _repaidOnPool > 0 || _withdrawnFromPool > 0;
        if (updated) {
            PoolInterestsVars memory interestsVars;
            (ratesVars.indexes.poolSupplyIndex, ratesVars.indexes.poolBorrowIndex, interestsVars) =
                _accruePoolInterests(IVToken(_poolToken));

            interestsVars.cash =
                interestsVars.cash + _suppliedOnPool + _repaidOnPool - _borrowedFromPool - _withdrawnFromPool;
            interestsVars.totalBorrows = interestsVars.totalBorrows + _borrowedFromPool - _repaidOnPool;

            IInterestRateModel interestRateModel = IVToken(_poolToken).interestRateModel();
            ...
            if (result.length > 32) {
                (, poolBorrowRate) = abi.decode(result, (uint256, uint256));
            } else {
                poolBorrowRate = abi.decode(result, (uint256));
            }
        } else {
            ratesVars.indexes.poolSupplyIndex = IVToken(_poolToken).exchangeRateStored();
            ratesVars.indexes.poolBorrowIndex = IVToken(_poolToken).borrowIndex();

            poolSupplyRate = IVToken(_poolToken).supplyRatePerBlock();
            poolBorrowRate = IVToken(_poolToken).borrowRatePerBlock();
        }

        (ratesVars.indexes.p2pSupplyIndex, ratesVars.indexes.p2pBorrowIndex, ratesVars.params) = _computeP2PIndexes(
            _poolToken,
            updated,
        ...
        uint256 p2pRate = PercentageMath.weightedAvg(poolSupplyRate, poolBorrowRate, ratesVars.params.p2pIndexCursor);
    ...
    function _getMarketSupply(
        ...
        p2pSupplyAmount =
            _delta.p2pSupplyAmount.mul(_p2pSupplyIndex).zeroFloorSub(_delta.p2pSupplyDelta.mul(_poolSupplyIndex));
        poolSupplyAmount = IVToken(_poolToken).balanceOf(address(evoq)).mul(_poolSupplyIndex);
    }
    ...
    function _getMarketBorrow(
        ...
        p2pBorrowAmount =
            _delta.p2pBorrowAmount.mul(_p2pBorrowIndex).zeroFloorSub(_delta.p2pBorrowDelta.mul(_poolBorrowIndex));
        poolBorrowAmount = IVToken(_poolToken).borrowBalanceStored(address(evoq)).div(IVToken(_poolToken).borrowIndex())
    ...
    function _getUserSupplyRatePerBlock(
        ...
        (uint256 p2pSupplyRate,, uint256 poolSupplyRate,) =
            _getRatesPerBlock(_poolToken, _suppliedOnPool, 0, 0, _repaidToPool);
    ...
    function _getUserBorrowRatePerBlock(
        ...
        (, uint256 p2pBorrowRate,, uint256 poolBorrowRate) =
            _getRatesPerBlock(_poolToken, 0, _borrowedFromPool, _withdrawnFromPool, 0);
        ...
        returns (uint256 weightedRate, uint256 totalBalance)
    {
        ...
        if (_balanceInP2P > 0) {
            weightedRate += _p2pRate.mul(_balanceInP2P.div(totalBalance));
        }
        if (_balanceOnPool > 0) {
            weightedRate += _poolRate.mul(_balanceOnPool.div(totalBalance));
        }
    }
}
