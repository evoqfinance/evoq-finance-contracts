...
abstract contract UsersLens is IndexesLens {
    ...
    struct UserHypotheticalBalanceStatesVar {
        address _user;
        address _poolToken;
    ...
    function getEnteredMarkets(address _user) external view returns (address[] memory enteredMarkets) {
        return evoq.getEnteredMarkets(_user);
    }
        ...
        returns (uint256 withdrawable, uint256 borrowable)
    {
        ...
        address[] memory enteredMarkets = evoq.getEnteredMarkets(_user);
        ...
        uint256 poolTokenBalance = _poolToken == evoq.vBnb()
            ? _poolToken.balance
            : ERC20(IVToken(_poolToken).underlying()).balanceOf(_poolToken);

        borrowable = Math.min(poolTokenBalance, (data.maxDebtUsd - data.debtUsd).div(assetData.underlyingPrice));
        withdrawable = Math.min(poolTokenBalance, assetData.collateralUsd.div(assetData.underlyingPrice));
    ...
    function computeLiquidationRepayAmount(
        ...
        if (!isLiquidatable(_user, _poolTokenBorrowed, updatedMarkets)) {
            return 0;
        }

        IVenusOracle venusOracle = IVenusOracle(comptroller.oracle());

        (,, uint256 totalCollateralBalance) = getCurrentSupplyBalanceInOf(_poolTokenCollateral, _user);
        (,, uint256 totalBorrowBalance) = getCurrentBorrowBalanceInOf(_poolTokenBorrowed, _user);

        uint256 borrowedPrice = venusOracle.getUnderlyingPrice(_poolTokenBorrowed);
        uint256 collateralPrice = venusOracle.getUnderlyingPrice(_poolTokenCollateral);
        if (borrowedPrice == 0 || collateralPrice == 0) {
    ...
    /// @param _suppliedAmount The amount to hypothetically supply to the given market (in underlying).
    ...
    /// @param _repaidAmount The amount to hypothetically repay to the given market (in underlying).
    ...
    function getUserHypotheticalBalanceStates(
        ...
        UserHypotheticalBalanceStatesVar memory vars = UserHypotheticalBalanceStatesVar(
            _user, _poolToken, _suppliedAmount, _withdrawnAmount, _borrowedAmount, _repaidAmount
        );

        IVenusOracle oracle = IVenusOracle(comptroller.oracle());
        address[] memory createdMarkets = evoq.getAllMarkets();
        ...
        for (uint256 i; i < nbCreatedMarkets; ++i) {
            ...
            Types.AssetLiquidityData memory assetData = _poolToken == poolToken
                ? _getUserHypotheticalLiquidityDataForAsset(
                    ...
                    true,
                    oracle,
                    vars._suppliedAmount,
                    vars._withdrawnAmount,
                    vars._borrowedAmount,
                    vars._repaidAmount
                )
                : _getUserHypotheticalLiquidityDataForAsset(vars._user, poolToken, true, oracle, 0, 0, 0, 0);
        ...
        returns (uint256 collateralUsd, uint256 debtUsd, uint256 maxDebtUsd)
    {
        ...
        for (uint256 i; i < enteredMarkets.length;) {
            ...
            Types.AssetLiquidityData memory assetData =
                getUserLiquidityDataForAsset(_user, poolTokenEntered, shouldUpdateIndexes, oracle);
    ...
    function getUserLiquidityDataForAsset(
        ...
        IVenusOracle _oracle
    ) public view returns (Types.AssetLiquidityData memory) {
        return _getUserHypotheticalLiquidityDataForAsset(_user, _poolToken, _getUpdatedIndexes, _oracle, 0, 0, 0, 0);
    }
    ...
    function isLiquidatable(address _user, address _poolToken, address[] memory _updatedMarkets)
        ...
        returns (bool)
    {
        if (evoq.marketPauseStatus(_poolToken).isDeprecated) {
    ...
    function _getUserSupplyBalanceInOf(
        ...
        return supplyBalance.inP2P.mul(_p2pSupplyIndex) + supplyBalance.onPool.mul(_poolSupplyIndex);
    }
    ...
    function _getUserBorrowBalanceInOf(
        ...
        return borrowBalance.inP2P.mul(_p2pBorrowIndex) + borrowBalance.onPool.mul(_poolBorrowIndex);
    }
    ...
    function _getUserHypotheticalLiquidityDataForAsset(
        ...
        assetData.underlyingPrice = _oracle.getUnderlyingPrice(vars._poolToken);
        if (assetData.underlyingPrice == 0) revert VenusOracleFailed();

        (, assetData.collateralFactor,) = comptroller.markets(vars._poolToken);

        (, Types.Indexes memory indexes) = _getIndexes(vars._poolToken, _getUpdatedIndexes);

        assetData.collateralUsd = (
            _getUserSupplyBalanceInOf(vars._poolToken, vars._user, indexes.p2pSupplyIndex, indexes.poolSupplyIndex)
                + vars._suppliedAmount
        ).zeroFloorSub(vars._withdrawnAmount).mul(assetData.underlyingPrice);

        assetData.debtUsd = (
            _getUserBorrowBalanceInOf(vars._poolToken, vars._user, indexes.p2pBorrowIndex, indexes.poolBorrowIndex)
                + vars._borrowedAmount
        ).zeroFloorSub(vars._repaidAmount).mul(assetData.underlyingPrice);
