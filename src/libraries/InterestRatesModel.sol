...
library InterestRatesModel {
    ...
    struct P2PIndexComputeParams {
        ...
        uint256 p2pGrowthFactor; // Evoq's peer-to-peer median index growth factor (in wad).
    ...
    function computeGrowthFactors(
        ...
        growthFactors.poolSupplyGrowthFactor = _newPoolSupplyIndex.div(_lastPoolIndexes.lastSupplyPoolIndex);
        growthFactors.poolBorrowGrowthFactor = _newPoolBorrowIndex.div(_lastPoolIndexes.lastBorrowPoolIndex);
        ...
        if (growthFactors.poolSupplyGrowthFactor <= growthFactors.poolBorrowGrowthFactor) {
            uint256 p2pGrowthFactor = PercentageMath.weightedAvg(
                growthFactors.poolSupplyGrowthFactor, growthFactors.poolBorrowGrowthFactor, _p2pIndexCursor
            );

            growthFactors.p2pSupplyGrowthFactor =
                p2pGrowthFactor - (p2pGrowthFactor - growthFactors.poolSupplyGrowthFactor).percentMul(_reserveFactor);
            growthFactors.p2pBorrowGrowthFactor =
                p2pGrowthFactor + (growthFactors.poolBorrowGrowthFactor - p2pGrowthFactor).percentMul(_reserveFactor);
        } else {
            // The case poolSupplyGrowthFactor > poolBorrowGrowthFactor happens because someone sent underlying tokens to the
            // vToken contract: the peer-to-peer growth factors are set to the pool borrow growth factor.
    ...
    function computeP2PIndex(P2PIndexComputeParams memory _params) internal pure returns (uint256 newP2PIndex) {
        if (_params.p2pAmount == 0 || _params.p2pDelta == 0) {
            ...
            uint256 shareOfTheDelta = Math.min(
                (_params.p2pDelta.mul(_params.lastPoolIndex)).div((_params.p2pAmount).mul(_params.lastP2PIndex)),
    ...
    {
        p2pSupplyRate = _params.p2pRate - (_params.p2pRate - _params.poolRate).percentMul(_params.reserveFactor);

        if (_params.p2pDelta > 0 && _params.p2pAmount > 0) {
            uint256 shareOfTheDelta = Math.min(
                _params.p2pDelta.mul(_params.poolIndex).div(_params.p2pAmount.mul(_params.p2pIndex)),
                CompoundMath.WAD // To avoid shareOfTheDelta > 1 with rounding errors.
            );

            p2pSupplyRate =
                p2pSupplyRate.mul(CompoundMath.WAD - shareOfTheDelta) + _params.poolRate.mul(shareOfTheDelta);
        }
    }
    ...
    {
        p2pBorrowRate = _params.p2pRate + (_params.poolRate - _params.p2pRate).percentMul(_params.reserveFactor);
        ...
        if (_params.p2pDelta > 0 && _params.p2pAmount > 0) {
            ...
            p2pBorrowRate =
                p2pBorrowRate.mul(CompoundMath.WAD - shareOfTheDelta) + _params.poolRate.mul(shareOfTheDelta);
        }
    }
}
