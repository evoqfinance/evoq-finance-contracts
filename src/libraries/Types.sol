...
/// @dev Common types and structs used in Evoq contracts.
library Types {
    /// ENUMS ///
    ...
    struct BorrowBalance {
        ...
        uint256 onPool; // In pool borrow unit, a unit that grows in value, to keep track of the debt increase when borrowers are on Venus. Multiply by the pool borrow index to get the underlying amount.
    }
    ...
    // Variables are packed together to save gas (will not exceed their limit during Evoq's lifetime).
    ...
    struct MarketParameters {
        uint16 reserveFactor; // Proportion of the interest earned by users sent to the Treasury for each market, in basis point (100% = 10 000). The value is set at market creation.
