...
contract Utils is Test {
    ...
    function testEquality(uint256 _firstValue, uint256 _secondValue) internal pure {
        assertApproxEqAbs(_firstValue, _secondValue, 20);
    }

    function testEquality(uint256 _firstValue, uint256 _secondValue, string memory err) internal pure {
        assertApproxEqAbs(_firstValue, _secondValue, 20, err);
    }

    function testEqualityLarge(uint256 _firstValue, uint256 _secondValue) internal pure {
        assertApproxEqAbs(_firstValue, _secondValue, 1e16);
    }

    function testEqualityLarge(uint256 _firstValue, uint256 _secondValue, string memory err) internal pure {
        assertApproxEqAbs(_firstValue, _secondValue, 1e16, err);
    }

    /// @dev venus track balances deposited by dividing the amount by a rate to obtain vToken Units.
    ...
    ///      deposited. This function is useful to determine venus's users balances.
    function getBalanceOnVenus(uint256 _amountInUnderlying, uint256 _rate) internal pure returns (uint256) {
        uint256 vTokenAmount = (_amountInUnderlying * 1e18) / _rate;
        return ((vTokenAmount * _rate) / 1e18);
    }
    ...
    function _computeCompoundedInterest(uint256 _rate, uint256 _elapsedBlocks) internal pure returns (uint256) {
        ...
        return WAD + _rate * _elapsedBlocks + (_elapsedBlocks * (_elapsedBlocks - 1) * ratePowerTwo) / 2
            + (_elapsedBlocks * (_elapsedBlocks - 1) * (_elapsedBlocks - 2) * ratePowerThree) / 6;
    }
